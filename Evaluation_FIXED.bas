...
Public Sub EvaluateAVLStatus()
    ...
    wsResults.Range("A1:L1").value = Array( _
        ...
        "Driv P1", "Driv Target", "Driv Tested", "Driv Status", _
        "Resp P1", "Resp Target", "Resp Tested", "Resp Status", "Final Status")
    ...
    For i = 5 To lastRow
        ...
        If Trim(CStr(opCode)) <> "" Then
            ...
            drivTarget = ToDbl(wsSheet1.Cells(i, 8).value)
            drivTested = ToDbl(wsSheet1.Cells(i, 10).value)

            respTarget = ToDbl(wsSheet1.Cells(i, 15).value)
            respTested = ToDbl(wsSheet1.Cells(i, 16).value)
    ...
    MsgBox "Evaluation complete. Results written to sheet: " & wsResults.Name, vbInformation
End Sub
